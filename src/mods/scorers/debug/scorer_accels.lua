...
function sc_Accels_RemoveNote(tid)
    ...
    if Diff.instance.debugAccelDisplay then
...
function sc_Accels_RefreshScore()
	local final = 0
	local isMax = Diff.instance.debugAccelMode == "Max"
	local isAvg = Diff.instance.debugAccelMode == "Avg"
	local isCurrent = Diff.instance.debugAccelMode == "Current"
	local fullCount = 0
	local max = math.max
        ...
        EventHandler.instance:on(Events.NOTE, function(ev)
				...
				if ev.data:hasHands(Note.HandTypes.LEFT) then
                    ...
                    if prevAccelLeft > 0 then
                        ...
                        sc_Accels_EventIDs[sc_Accels_Count] = Intervals.instance:addInterval(Diff.instance.debugAccelTimer, true, sc_Accels_RemoveNote, {sc_Accels_Count})
